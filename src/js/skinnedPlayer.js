...
// isBlinking: if true, draw eyes as closed (horizontal lines)

const generateDefaultSkin = (expression = "neutral", isBlinking = false) => {
	...
	// Draw eyes - closed if blinking, otherwise open
	if (isBlinking) {
		// Closed eyes - just horizontal lines at fixed position
		rect(FACE_X + 2, FACE_Y + 3, 2, 1, ...BROWN)    // left eye closed
		rect(FACE_X + 5, FACE_Y + 3, 2, 1, ...BROWN)    // right eye closed
	} else {
		// Eyes (row 2-3, columns 2-3 and 5-6) - 2x2 pixels
		rect(FACE_X + 2, FACE_Y + 2, 2, 2, ...EYE_WHITE)   // left eye white
		rect(FACE_X + 2, FACE_Y + 2, 1, 1, ...EYE_BLUE)    // left iris
		set(FACE_X + 2, FACE_Y + 2, ...EYE_PUPIL)          // left pupil
		
		rect(FACE_X + 5, FACE_Y + 2, 2, 2, ...EYE_WHITE)   // right eye white
		rect(FACE_X + 5, FACE_Y + 2, 1, 1, ...EYE_BLUE)    // right iris
		set(FACE_X + 5, FACE_Y + 2, ...EYE_PUPIL)          // right pupil
		
		// Expression adjustments - subtle changes from baseline (2x2 eyes)
		if (expression === "surprised") {
		// Slightly taller eyes, pupils moved up
		rect(FACE_X + 2, FACE_Y + 1, 2, 3, ...EYE_WHITE)   // slightly taller (3px instead of 2px)
		rect(FACE_X + 5, FACE_Y + 1, 2, 3, ...EYE_WHITE)
		rect(FACE_X + 2, FACE_Y + 1, 1, 1, ...EYE_BLUE)    // iris same size, moved up
		rect(FACE_X + 5, FACE_Y + 1, 1, 1, ...EYE_BLUE)
		set(FACE_X + 2, FACE_Y + 1, ...EYE_PUPIL)          // pupil at top
		set(FACE_X + 5, FACE_Y + 1, ...EYE_PUPIL)
	} else if (expression === "angry") {
		// 2x2 eyes, eyebrows lowered
		rect(FACE_X + 2, FACE_Y + 2, 2, 2, ...EYE_WHITE)   // 2x2 eyes
		rect(FACE_X + 5, FACE_Y + 2, 2, 2, ...EYE_WHITE)
		rect(FACE_X + 2, FACE_Y + 2, 1, 1, ...EYE_BLUE)    // iris same size
		rect(FACE_X + 5, FACE_Y + 2, 1, 1, ...EYE_BLUE)
		set(FACE_X + 2, FACE_Y + 2, ...EYE_PUPIL)
		set(FACE_X + 5, FACE_Y + 2, ...EYE_PUPIL)
		// Eyebrows slightly lowered
		rect(FACE_X + 1, FACE_Y + 1, 2, 1, ...BROWN_D)     // darker, same position
		rect(FACE_X + 5, FACE_Y + 1, 2, 1, ...BROWN_D)
	} else if (expression === "sad") {
		// Eyes slightly lower, eyebrows slightly raised
		rect(FACE_X + 2, FACE_Y + 3, 2, 2, ...EYE_WHITE)   // 2x2, shifted down 1px
		rect(FACE_X + 5, FACE_Y + 3, 2, 2, ...EYE_WHITE)
		rect(FACE_X + 2, FACE_Y + 3, 1, 1, ...EYE_BLUE)    // iris same size, shifted down
		rect(FACE_X + 5, FACE_Y + 3, 1, 1, ...EYE_BLUE)
		set(FACE_X + 2, FACE_Y + 3, ...EYE_PUPIL)
		set(FACE_X + 5, FACE_Y + 3, ...EYE_PUPIL)
		// Eyebrows slightly raised
		rect(FACE_X + 1, FACE_Y + 0, 2, 1, ...BROWN)       // moved up 1px
		rect(FACE_X + 5, FACE_Y + 0, 2, 1, ...BROWN)
	} else if (expression === "happy") {
		// 2x2 eyes, eyebrows slightly raised
		rect(FACE_X + 2, FACE_Y + 2, 2, 2, ...EYE_WHITE)   // 2x2 eyes
		rect(FACE_X + 5, FACE_Y + 2, 2, 2, ...EYE_WHITE)
		rect(FACE_X + 2, FACE_Y + 2, 1, 1, ...EYE_BLUE)    // iris same size
		rect(FACE_X + 5, FACE_Y + 2, 1, 1, ...EYE_BLUE)
	...
	// Mouth (rows 5-6) - consistent 4-pixel width, subtle shape changes
	if (expression === "happy") {
		// Smile - same width, curves up slightly
		rect(FACE_X + 2, FACE_Y + 6, 4, 1, ...MOUTH)    // same width as neutral
		set(FACE_X + 1, FACE_Y + 5, ...MOUTH_D)         // slight upward curve
		set(FACE_X + 6, FACE_Y + 5, ...MOUTH_D)
	} else if (expression === "sad") {
		// Frown - same width, curves down slightly
		rect(FACE_X + 2, FACE_Y + 6, 4, 1, ...MOUTH)    // same width as neutral
		set(FACE_X + 1, FACE_Y + 7, ...MOUTH_D)         // slight downward curve
		set(FACE_X + 6, FACE_Y + 7, ...MOUTH_D)
	} else if (expression === "surprised") {
		// Open mouth - same width, opens slightly
		rect(FACE_X + 2, FACE_Y + 6, 4, 1, ...MOUTH)    // same width
		rect(FACE_X + 3, FACE_Y + 5, 2, 1, ...MOUTH_D)  // slight opening above
		rect(FACE_X + 3, FACE_Y + 7, 2, 1, ...MOUTH_D)  // slight opening below
	} else if (expression === "angry") {
		// Tight line - same width, slightly darker
		rect(FACE_X + 2, FACE_Y + 6, 4, 1, ...MOUTH_D)   // same width, darker
	} else {
		// Neutral - baseline
		rect(FACE_X + 2, FACE_Y + 6, 4, 1, ...MOUTH)    // neutral line
	}
...
class SkinnedPlayer extends Entity {
	constructor(x, y, z, skinTexture, glExtensions, gl, glCache, indexBuffer, world, p) {
		...
		this.nextBlinkTime = 100 + random() * 300  // 5-20 seconds at 20 TPS (100-400 frames)
	...
	_updateSkinTexture() {
		// Regenerate skin with current expression and blink state
		const newPixels = generateDefaultSkin(this.currentExpression, this.isBlinking)
		const gl = this.gl
	...
	_updateBlink(dt) {
		if (this.isBlinking) {
			// Currently blinking - count down blink duration
			this.blinkDuration -= dt
			if (this.blinkDuration <= 0) {
				// Blink finished - open eyes
				this.isBlinking = false
				// Schedule next blink (5-20 seconds)
				this.nextBlinkTime = 100 + random() * 300  // 100-400 frames at 20 TPS
				this.blinkTimer = 0
				this._updateSkinTexture()
			}
		} else {
			...
			if (this.blinkTimer >= this.nextBlinkTime) {
				...
				this.blinkDuration = 3  // Blink for ~0.15 seconds at 20 TPS
				this.blinkTimer = 0
				this._updateSkinTexture()
			}
		}
